...
public class UserService {
    ...
    public Map<String, Object> login(String userId, String password){
        //userName없음
        User selectedUser = userRepository.findByUserName(userId)
                .orElseThrow(()->new AppException(userId + "이 없습니다.", ErrorCode.USERNAME_NOT_FOUND));
        ...
        String token = JwtUtil.createJwt(Id,userId,name,email,is_active,role,secretKey,expiredMs);

        //response에 유저정보담기 / 토큰에도 담겨있음
        Map<String,Object> response = new HashMap<>();
        response.put("id",selectedUser.getId());
        response.put("token",token);
        response.put("userId",selectedUser.getUserName());
