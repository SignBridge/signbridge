...
const ClientChatRoom = () => {
  ...
  const handlePrivateConnect = () => {
    ...
    client.connect({"111":"111"}, (frame) => {
      _from.current = frame.headers['user-name'];
      setFrom(_from.current);
  ...
  const sendMessage = useCallback(() => {
    ...
    console.log("sendMessagesendMessagesendMessagesendMessagesendMessage: ",from)
    if (!stompClient) {
      console.error("stompClient is not defined");
      return;
    }
    console.log("sendMessage함수 내 stompClient",stompClient)
    stompClient.send('/app/application', {}, JSON.stringify({ from : _from.current, text:  text}));
  }, [stompClient]);
  ...
  const sendPrivateMessage = () => {
    ...
    console.log(JSON.stringify({ text, username,from }))
    privateStompClient.send('/app/private', {}, JSON.stringify({ text: text,from : _from.current}));
  };
